    ...
    try{
        if(isset($_SESSION['profile_user_id'])){
            ...
            $stmt = $db->prepare(
                ...
                union all select c.post_id as post_id, count(likes.post_id) as likes, comments, shares from(

                    select b.post_id, count(comments.post_id) as comments, shares from(

                        select a.post_id, count(shares.post_id) as shares
                        from(select posts.post_id from posts 
                            left join favorites on favorites.post_id=posts.post_id 
                            where posts.user_id=? or favorites.user_id=?
                        ) as a
                ...
                left join likes on likes.post_id=c.post_id GROUP BY c.post_id
            ");
